...
DEFAULTS["accent"] = "f0b429"
DEFAULTS["text_sub"] = "8a8f98"
DEFAULTS["bar_track"] = "2d3138"
...
def generate(
    ...
    **kwargs,
    ...
    rank_w = draw.textlength(rank_text, font=f_stats)
    ...
    if fill_w > 0:
        draw.rounded_rectangle(
            [LEFT, BAR_Y, LEFT + fill_w, BAR_Y + BAR_H],
            radius=9,
            fill=ACCENT
        )

    xp_text = f"{xp_fmt(xp)} / {xp_fmt(xp_max)} XP"
    draw.text((LEFT, BAR_Y + BAR_H + 8), xp_text, font=f_xp, fill=TXT2)
...
CONFIG_PATH = os.path.join(os.path.dirname(__file__), "rankcard_configs.json")
...
def save_config(user_id: str, cfg: dict):
    ...
    configs[str(user_id)] = cfg
    with open(CONFIG_PATH, "w") as f: json.dump(configs, f, indent=2)

def get_config(user_id: str) -> dict:
    return load_configs().get(str(user_id), {})

def delete_config(user_id: str):
    configs = load_configs()
    configs.pop(str(user_id), None)
    with open(CONFIG_PATH, "w") as f: json.dump(configs, f, indent=2)